...
def doGeneration():
    ...
    for i, j, k in itertools.product(range(25), range(25), range(25)):
        value = arr[i, j, k]
        neighbours = np.sum(arr[i-1:i+2, j-1:j+2, k-1:k+2]) - value
...
arr = np.zeros((25, 25, 25))

arr[8:8+len(initialState), 8:8+len(initialState), 13] = initialState
